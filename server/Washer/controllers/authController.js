...
const handleErrors=(err)=>{
    let error= {email:'', password:'', isApproved: ''};
    ...
    //Washer not approved
    if(err.message === 'Admin approval is pending'){
        error.isApproved ="Admin approval is pending for your account";
    }
    ...
    if(err.message.includes('washers validation failed'))
    {
...
const createToken =(id)=>{
    return jwt.sign({ id }, 'A strong secret token here',{
        expiresIn: maxAge
    })
}
...
module.exports.get_signup = function(req,res){
    ...
    res.send("hi this is washers signup page");
}
